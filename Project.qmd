...
hiv = hiv.applymap(lambda x: x.replace('k', '000') if isinstance(x, str) else x)
hiv = hiv.apply(pd.to_numeric, errors='coerce')
malaria = malaria.applymap(lambda x: x.replace('k', '000') if isinstance(x, str) else x)
...
### select the country to view {.sidebar}
...
###
```{python}
# Function to plot data for a specific country
def plot_data(country):
    ...
    plt.title(f'Data for Country {country}')
    plt.xlabel('Year')
    plt.ylabel('Value')
    plt.grid(True)
    plt.show()
